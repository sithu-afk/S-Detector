...
Real-time phishing site detector for Firefox. Independent from S_Guard (ad blocker) — separate codebase, separate extension ID, separate AMO listing.

[![Lint extension](https://github.com/sithu-afk/S_Detector/actions/workflows/lint.yml/badge.svg)](https://github.com/sithu-afk/S_Detector/actions/workflows/lint.yml)
...
See [`AMO_SUBMISSION.md`](AMO_SUBMISSION.md) for the full submission checklist, ready-to-paste listing text, and reviewer notes explaining the `<all_urls>` permission request. Also see [`PRIVACY_POLICY.md`](PRIVACY_POLICY.md), which AMO requires since this extension checks URLs against an external service.

Quick summary:
- Replace placeholder icons with final artwork
- Decide how to handle the Safe Browsing API key before public release (see "Important: API key handling" in `AMO_SUBMISSION.md`) — a hardcoded key isn't safe once the extension is public
- Test the allowlist flow and options page toggles end-to-end
- Run `web-ext lint` and fix any warnings

## Security

See [`SECURITY.md`](SECURITY.md) for a summary of the security self-review performed on this codebase, including findings and fixes (safe DOM construction, message sender validation, feed data validation, explicit CSP) and how to report a vulnerability.
